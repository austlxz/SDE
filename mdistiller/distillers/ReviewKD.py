...
from mdistiller.distillers.SDE import multi_dkd
...
class ReviewKD(Distiller):
    ...
    def forward_train(self, image, target, **kwargs):
        logits_student, features_student, patch_s = self.student(image)
        with torch.no_grad():
            logits_teacher, features_teacher, patch_t = self.teacher(image)
        ...
        loss_multi_dkd = multi_dkd(patch_s, patch_t, target, 1, 8, 4)

        losses_dict = {
            "loss_ce": loss_ce,
            "loss_kd": loss_reviewkd + loss_multi_dkd,
        }
